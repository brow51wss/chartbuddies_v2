...
| LIVE-007 | closed | high | `/profile`, `/patients/[id]/mar/[marId]`, `/patients/[id]/progress-notes/view` | Signature and initials images broken (showing broken icon or fallback text) on live site. | All users with signatures set up | 2026-06-10 | dev | See §12 resolution note. |
| LIVE-008 | fixing | high | Edit Patient modal (patient photo section) | "Patient not found or access denied" when tapping the phone icon to send a photo capture email. Root cause: `pages/api/send-patient-photo-capture-email.ts` queries `patients` from Supabase — table has been migrated to RDS. | All users trying to add patient photo via phone | 2026-06-11 | dev | Fix: switch to `rdsGetPatient` from `lib/rdsApi.ts`. |
